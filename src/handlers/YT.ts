...
import { getDbPath, getUnixTimestamp, isPendleAddress } from "../helper.ts";
import { readAllUserERC20Balances, readAllYTPositions } from "../multicall.ts";
import { EVENT_USER_SHARE, POINT_SOURCE_YT, PointAmounts, UPDATE_TYPE_USER_YT_POSITION } from "../types.ts";
import {
  calcPointsFromHolding,
  updateUserPoint,
} from "../points/point-manager.ts";
import { getERC20ContractOnContext } from "@sentio/sdk/eth/builtin/erc20";
import { YTAccountSnapshot } from "../schema/schema.ts"
...
export async function updateAllYTAccounts(
  ...
  const allAddresses = shouldIncludeDb
    ? (await ctx.store.list(YTAccountSnapshot, [])).map((snapshot) => (snapshot.id.toString().toLowerCase()))
    : [];
  ...
  await Promise.all(
    allAddresses.map(async (address, i) => {
      ...
      const snapshot = await ctx.store.get(YTAccountSnapshot, address);
      if (snapshot && snapshot.lastUpdatedAt < timestamp) {
        ...
        await updateUserPoint(ctx, address, POINT_SOURCE_YT, {
      ...
      const newSnapshot: YTAccountSnapshot = new YTAccountSnapshot({
        id: address,
        lastUpdatedAt: timestamp,
        lastImpliedHolding: impliedHolding,
      });
